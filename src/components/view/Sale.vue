         ...
         <div class="cmp_cont">
             ...
             <div class="cmp_b_c1">
                 ...
                 <el-form ref="form" :model="form" :rules="rules" label-width="80px">
                    ...
                    <el-form-item label="卖出价格" prop="salePrice" size="mini">
                      <el-input type="number" onmousewheel="return false;" v-model="form.salePrice" clearable></el-input>
                    </el-form-item>
                    <el-form-item label="可卖股数" prop="canSaleNum" size="mini">
                      <el-input v-model="form.canSaleNum" disabled></el-input>
                    </el-form-item>
                    <el-form-item label="卖出股数" prop="saleNum" size="mini">
                      <el-col :span="15">  
                       <el-input type="number" onmousewheel="return false;" v-model="form.saleNum" clearable></el-input>
                      </el-col>
                      <el-col :span="6" :offset="1">  
                       <el-button style="border-style:none;color:#2D8DDF" @click="dianMe">全部</el-button>
                      </el-col>
                    </el-form-item>
                    ...
                    <el-form-item>
                      <el-button @click="submitForm('form')" style="background:#0047AB;color:#fff;float:left;margin-top:7px;" size="mini">卖出</el-button>
                    </el-form-item>
                </el-form>
                <!--显示的相关的数据-->
       ...
       <div class="buy-d2">
          <div class="cmp_cont">
              <div class="cmp_b_c2">
                 ...
                 <div class="cmp_b_S" style="margin-top:8px;">
...
//					startValue:100000,//初期金额
        ...
        rules:{
          stockStr:[
              { required: true, message: '请输入股票代码或名称', trigger: 'change' }
          ],
          salePrice: [
...
　　　　             }else { 
                       ...
                       if(bai){
                         if(value*1 > vm.form.canSaleNum*1){
                             callback('卖出数量大于可卖数量');
                             return false;
                         }
                         callback();
...
　　　　            } 
  ...
  methods: {
        ...
        vm.$refs[formName].validate((valid) => {
            if(valid){
                 vm.$confirm('<div class="d1"><p><span>证券名称：</span><span>'+vm.form.stockName+'</span></p><p><span>证券代码：</span><span>'+vm.form.stockCode+'</span></p></div>' +
                            '<div class="d1"><p><span>委托价格：</span><span>'+vm.form.salePrice+'</span></p><p><span>委托数量：</span><span>'+vm.form.saleNum+'</span></p></div>' +
                         ...
                         type: ''
                            ...
                            setStatuc(0);
                            APIS.saleSubmit(vm.form.stockCode,vm.form.salePrice,vm.form.saleNum).then((res) => {
                                        if(res.code == 0) {
                                             this.$message({
                                               message: '卖出成功',
                                               type: 'success'
     ...
     getBs(stockCode) {
        return new Promise((resolve, reject) => {
            ...
            //  console.log(arr[0].substring(1,3));
             ...
             arr.push(down);
            //给卖出金额第一次赋值。
             if(vm.firSalePrice == 0){
                 vm.form.salePrice = (arr[3]*1).toFixed(2);
                 vm.firSalePrice = 1;
             }
             ...
             if(vm.optionDex == 0){
                ...
                if(vm.$refs.drawTime.mychart1 != undefined){
                      if(vm.ntime == 0){
                ...
                if(vm.$refs.drawKline.mychart2 != undefined){
                   if(vm.nday == 0) {
    ...
    //点击全部的效果
    dianMe() {
      vm.form.saleNum = vm.form.canSaleNum;
    },
    //获取持仓的值
   positionCommon() {
       return new Promise((resolve,reject) => {
				...
				APIS.positionCommon().then(function(res) {
                  if(res.code != 0){
                   ...
                   //console.log(positionCommonData);
                   ...
                   if(positions.length == 0) {
                       vm.form.canSaleNum = 0;
                       vm.form.stocknumber = 0;//当前持仓的数量
                   }else{
                      //判断是否有该股票 
                      ...
                      if(jnj == -1){
                          ...
                          let numberCanUse = positions[jnj].number - positions[jnj].numberFreeze;
                          vm.form.canSaleNum = numberCanUse;
                          vm.form.stocknumber = positions[jnj].number; //获取当前持仓的数量
                      }
                      
                   }
                     //给预计的金额赋值
                     let val = vm.form.salePrice*vm.form.saleNum;
                     if(isNaN(val)){
